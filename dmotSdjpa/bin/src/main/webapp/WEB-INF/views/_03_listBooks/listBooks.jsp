...
<%--      src="<c:url value='/_00_init/getBookImage?id=${entry.value.id}' />"> --%>
        <img height='100' width='80' src="${entry.value.coverImage}">
         </TD>
         ...
         <TD rowspan='3' width='180'align='center'>
         	<c:choose>
         		<c:when test="${entry.value.stock > 0}">
                   ...
                   -->
                     <FORM  action="<c:url value='/_03_listBooks/buyBook' />" 
                            method="POST">
                                購買數量:
                        <select name='qty'>
                       ...
                       <!-- 這些隱藏欄位都會送到後端 -->
                       <Input type='hidden' name='bookId' value='${entry.value.id}'>
       ...
       <TD width='560'>
         <TABLE border='1'>
            <TR height='31'>
               ...
               <TD width='140'>出版社：${fn:substring(entry.value.publisher.name,0,2)}</TD>
            </TR>
         </TABLE>
      </TD>
   </TR>
...
<div id='content' style="position:fixed; top:450px; left:280px;">
    ...
    <td width='76'>
     ...
     <td width='76'>
            <c:if test="${pageNo != totalPages}">
                <div id="pnext">
                   <a href="<c:url value='/_03_listBooks/DisplayPageProducts?pageNo=${pageNo+1}' />">下一頁</a>
                </div>
            </c:if>
     </td>  
     <td width='76'>
            <c:if test="${pageNo != totalPages}">
                <div id="plast">
                    <a href="<c:url value='/_03_listBooks/DisplayPageProducts?pageNo=${totalPages}' />">最末頁</a>
                </div>
            </c:if>
     </td>
