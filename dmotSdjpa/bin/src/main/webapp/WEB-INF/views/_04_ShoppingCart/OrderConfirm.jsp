...
function cancelOrder() {
	if (confirm("確定取消此份訂單 ? ") ) {
		...
		document.forms[0].action="<c:url value='/_04_ShoppingCart/processOrder' />";
...
<h3>請確認下列訊息：</h3>
<FORM style="margin: 0 auto; width:750px;" action="<c:url value='/_04_ShoppingCart/processOrder' />" method="POST" >
...
<%--          	訂單日期：<fmt:formatDate value="${SYSTEM.utilDate}" pattern="yyyy-MM-dd"/> --%>
      ...
      <TR>
         <TD colspan='3' style="text-align:left; border-color: #FFBD32; border-style: ridge;">
                   統一編號：<Input style="background:#ECFFCD;" maxlength="8" type="text" 
                   name="BNO" value="">
         </TD>
      </TR>
      <TR>
   ...
   <TABLE border='1' style="background:#FFE7CD; border-color:rgb( 100, 100, 255); " >
     ...
     <c:forEach varStatus="vs" var="entry" items="${ShoppingCart.content}">
        ...
        <TR height='16'>
          ...
          <TD style="text-align:center;font-size: 11pt;">
          	${fn:substring(entry.value.publisherName, 0, 2)}
          </TD>
